...
- **Service Logic (`deleteFile`):**
  ...
  4. **Atomic DB delete (transaction):** Inside `withTransaction`, runs `File.deleteOne({ _id, userId }, { session })` plus `updateAncestorDirectoryStats(file.parentDirId, { bytes: -file.size, files: -1 }, session)` — decrementing the parent folder and every ancestor's denormalized `size`/`fileCount`. `endSession()` runs in `finally`.
