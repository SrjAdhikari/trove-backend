...
- **Service Logic (`uploadFile`):**
  ...
  8. **Atomic persist (transaction):** With the final byte count known, opens `mongoose.startSession()` and inside `withTransaction`: first **enforces the per-user storage quota** — reads the root directory `size` and throws `AppError` with `BAD_REQUEST` and `STORAGE_LIMIT_EXCEEDED` if `usedBytes + bytes > storageLimit` — then runs `File.create([{ _id, size: bytes, ... }], { session })` plus `updateAncestorDirectoryStats(parentDir._id, { bytes, files: 1 }, session)`, bumping the parent folder and every ancestor's denormalized `size`/`fileCount`. Because the quota read shares the root document that `updateAncestorDirectoryStats` `$inc`s, concurrent uploads write-conflict on it and `withTransaction` retries the loser against the fresh size — the cap holds without a lock.
...
The `_id` is minted in application code up front (it's the physical filename), so the bytes can be streamed to disk **before** any DB row exists. Only after the stream succeeds does the record get created — together with the ancestor folder-size updates — inside a single transaction. This is strictly safer than the earlier "create row first, then stream" approach: a failed stream leaves nothing in the DB to roll back (just an `rm` of the partial file), and the row is born already carrying its final size.
