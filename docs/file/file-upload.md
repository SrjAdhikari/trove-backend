...
- **Service Logic (`uploadFile`):**
  ...
  8. **Atomic persist (transaction):** With the final byte count known, opens `mongoose.startSession()` and inside `withTransaction`: first **enforces the per-user storage quota** — reads the root directory `size` and throws `AppError` with `BAD_REQUEST` and `STORAGE_LIMIT_EXCEEDED` if `usedBytes + bytes > storageLimit` (PR #66) — then runs `File.create([{ _id, size: bytes, ... }], { session })` plus `adjustAncestorStats(parentDir._id, { bytes, files: 1 }, session)`, bumping the parent folder and every ancestor's denormalized `size`/`fileCount` (PR #62). Because the quota read shares the root document that `adjustAncestorStats` `$inc`s, concurrent uploads write-conflict on it and `withTransaction` retries the loser against the fresh size — the cap holds without a lock.
