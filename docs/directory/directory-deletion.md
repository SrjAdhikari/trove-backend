...
File and directory `deleteMany` operations run inside a single transaction. If either operation fails, both roll back — preventing partial deletes where files exist without their parent directory or vice versa.
