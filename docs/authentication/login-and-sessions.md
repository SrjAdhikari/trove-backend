...
- **Flow:**
  1. `validateBody(googleOAuthSchema)` ensures `idToken` is a non-empty string at the route (rejects with `400 VALIDATION_ERROR` otherwise).
...
- **Flow:**
  1. `validateBody(githubOAuthSchema)` ensures `code` is a non-empty string at the route (rejects with `400 VALIDATION_ERROR` otherwise).
