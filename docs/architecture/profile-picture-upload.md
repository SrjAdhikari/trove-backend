...
| Serving model | **Public capability URL** — a random unguessable token is the route `:id`; a public no-auth route streams by token | Robust across origins (prod FE is cross-origin, `SameSite=None`, so cookie-authed `<img>` is unreliable under third-party-cookie blocking). Non-enumerable. New token per upload gives free cache-busting. |
